...
def script_properties():
    ...
    obspython.obs_properties_add_int(pros, "delay", "结束延迟", 0, 999, 1)
...
def script_update(setting):
    ...
    delay = obspython.obs_data_get_int(setting, "delay")
    if enabled:
...
def update():
    ...
    if source:
        ...
        if test:
            ...
            obspython.script_log(obspython.LOG_INFO, "测试" + str(time.strftime("%Y%m%d%H%M%S", (time.localtime(int(time.mktime(timeArray)) + ptime)))))
            timed = str(20220316233503)
        else:
        ...
        obspython.script_log(obspython.LOG_INFO, "delay" + " " + str(delay))
        obspython.script_log(obspython.LOG_INFO, "delayt" + " " + str(delayt))
